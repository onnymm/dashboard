...
const GridItem = ({ icon: Icon, amount, label, percent, gain }) => {
	...
	return (
		<div
			className={`${sidebarIsLocked ? 'xl:col-span-3' : 'lg:col-span-3'} duration-dark col-span-12 h-40 w-full rounded bg-white px-7 py-6 shadow-md transition-colors dark:bg-navbar-background-d sm:col-span-6`}
		>
			{/* Ícono */}
			{Icon && (
				<div className='mb-3 flex'>
					<div className='duration-dark rounded-full bg-slate-100 p-3 opacity-80 transition-all dark:bg-navbar-icons-background-d'>
			...
			<div className='flex justify-between'>
				...
				<div className='flex flex-col gap-1'>
					<h1 className='duration-dark text-xl font-bold transition-all dark:text-white'>
						{amount}
					</h1>
					<p className='duration-dark text-sm opacity-60 transition-all dark:text-white'>
						{label}
					</p>
				</div>
				{/* Porcentaje y flecha */}
