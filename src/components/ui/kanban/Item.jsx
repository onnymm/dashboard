...
const Item = ({ id, title }) => {
	...
	return (
		<div
			...
			{...attributes}
			ref={setNodeRef}
			className={`w-full rounded-sm bg-white py-2 pl-2 text-left shadow transition-opacity ${isDragging && 'opacity-50'}`}
		>
			<div className='flex w-full items-center justify-between gap-1 text-slate-800'>
				<p className='flex w-72 text-sm [overflow-wrap:anywhere]'>{title}</p>
				{/* Handle para drag */}
				<button
					{...listeners}
